...
export default {
  ...
  methods: {
    async register() {
      try {
        const res = await fetch('http://localhost:8000/api/register-guru', {
          method: 'POST',
          headers: {
...
<template>
  <div class="bg-linear-180 from-indigo-600 via-indigo-50 to-indigo-600 min-w-full h-screen flex justify-center items-center">
    <div class="bg-gray-100 px-10 py-10 rounded-lg shadow-lg w-full max-w-4xl">
      <h2 class="mb-6 text-center font-bold text-2xl text-blue-600">REGISTER GURU</h2>
      ...
      <form @submit.prevent="register" class="grid grid-cols-2 gap-6">
        ...
        <div class="space-y-4">
          ...
          <div>
            ...
            <input v-model="form.id_number" placeholder="NIP" class="w-full border rounded px-3 py-2" />
          </div>
